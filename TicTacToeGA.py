...
f = open('Estrategia.txt','a')
print("The Percentage of Lost Games is = ", result.result * 100)
line = "The Percentage of Lost Games is = " + str(result.result * 100) + "\n"
f.write(line)
print("The Strategy is :")
line = "The Strategy is :" + "\n"
f.write(line)
bestAnswer = agent.Population[result.bestStrategy]
for i in range(len(bestAnswer)):
    print(i, " --> ", bestAnswer[i])
    line = "" + str(i) + " --> " + str(bestAnswer[i]) + "\n"
    f.write(line)
f.close()

f2 = open('MatrisBase.txt','a')
i = 0
for r in agent.baseStateMatrix:
    f2.write(str(i) + "   ")
    for c in r:
        if c == GridCell.X:
            print("1", end=" ")
