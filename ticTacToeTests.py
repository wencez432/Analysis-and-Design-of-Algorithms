...
# from chapter 18 of _Genetic Algorithms with Python_, an ebook
# available for purchase at http://leanpub.com/genetic_algorithms_with_python
#
# Author: Clinton Sheppard <fluentcoder@gmail.com>
# Repository: https://drive.google.com/open?id=0B2tHXnhOFnVkTXU5VWs2d1pXX3c
...
def play1on1(xGenes, oGenes):
    ...
    roundData = [[xGenes, ContentType.Mine, genetic.CompetitionResult.Loss, genetic.CompetitionResult.Win] , [oGenes, ContentType.Opponent, genetic.CompetitionResult.Win, genetic.CompetitionResult.Loss]]
    playerIndex = 0
    ...
    while len(empties)> 0:
        ...
        if len(RowContentFilter(piece, 3).get_matches(board, mostRecentMoveOnly)) > 0 or len(ColumnContentFilter(piece, 3).get_matches(board, mostRecentMoveOnly)) > 0 or len(DiagonalContentFilter(piece, 3).get_matches(board, mostRecentMoveOnly)) > 0:
            return winResult
        empties = [v for v in board.values() if v.Content == ContentType.Empty]
...
def get_fitness_for_games(genes):
    def getBoardString(b):
        return ''.join(map(lambda i:'.' if b[i].Content == ContentType.Empty else 'x' if b[i].Content == ContentType.Mine else 'o', squareIndexes))
    ...
    while len(queue) > 0:
        ...
        mostRecentMoveOnly = [board[index]]
        if len(iHaveThreeInRow.get_matches(board, mostRecentMoveOnly)) > 0 or len(iHaveThreeInColumn.get_matches(board, mostRecentMoveOnly)) > 0 or len(iHaveThreeInDiagonal.get_matches(board, mostRecentMoveOnly)) > 0:
        ...
        # if any empties have two OPPONENT pieces in ROW, COLUMN or DIAGONAL then we lost
        empties = [v for v in board.values() if v.Content == ContentType.Empty]
...
def create_geneset():
    ...
    geneset = [
        ...
        RuleMetadata(lambda expectedContent, count: MiddleRowFilter(), options),
        RuleMetadata(lambda expectedContent, count: BottomRowFilter(), options),
        RuleMetadata(ColumnContentFilter, options),
        RuleMetadata(lambda expectedContent, count: LeftColumnFilter(), options),
        RuleMetadata(lambda expectedContent, count: MiddleColumnFilter(), options),
        RuleMetadata(lambda expectedContent, count: RightColumnFilter(), options),
        RuleMetadata(DiagonalContentFilter, options),
        RuleMetadata(lambda expectedContent, count: DiagonalLocationFilter(), options),
        ...
        RuleMetadata(lambda expectedContent, count: RowOppositeFilter(expectedContent), options, needsSpecificContent=True),
        RuleMetadata(lambda expectedContent, count: ColumnOppositeFilter(expectedContent), options, needsSpecificContent=True),
        RuleMetadata(lambda expectedContent, count: DiagonalOppositeFilter(expectedContent), options, needsSpecificContent=True),
        ]
...
class RuleMetadata:
    ...
    def __init__(self, create, options=None, needsSpecificContent=True, needsSpecificCount=True):
        if options is None:
...
class ContentFilter(Rule):
    def __init__(self, description, expectedContent, expectedCount, getValueFromSquare):
    ...
    def get_matches(self, board, squares):
        ...
        for square in squares:
            m = list(map(lambda i: board[i].Content, self.getValueFromSquare(square)))
...
class RowContentFilter(ContentFilter):
    def __init__(self, expectedContent, expectedCount):
        super().__init__("its ROW has", expectedContent, expectedCount, lambda s: s.Row)

class ColumnContentFilter(ContentFilter):
    def __init__(self, expectedContent, expectedCount):
        super().__init__("its COLUMN has", expectedContent, expectedCount, lambda s: s.Column)

class LocationFilter(Rule):
    def __init__(self, expectedLocation, containerDescription, func):
        super().__init__("is in " + expectedLocation + " " + containerDescription)
        self.func = func
...
class DiagonalLocationFilter(LocationFilter):
    def __init__(self):
        super().__init__("DIAGONAL", "", lambda square: not (square.IsMiddleRow or square.IsMiddleColumn) or square.IsCenter)
    ...
    def get_matches(self, board, squares):
        ...
        for square in squares:
            if square.DiagonalOpposite is None:
                continue
            if board[square.DiagonalOpposite].Content == self.ExpectedContent: result.add(square.Index)
        return result
